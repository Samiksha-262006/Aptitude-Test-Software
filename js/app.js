...
const AppState = {
    ...
    previousScreenBeforeCert: 'dashboardView' // Tracks where the user came from before viewing certificate
};
...
function initAuthListeners() {
    ...
    if (studentRegisterForm) {
        studentRegisterForm.addEventListener('submit', (e) => {
            ...
            const securityQuestion = document.getElementById('regSecurityQuestion')?.value || 'What is your favorite subject?';
            ...
            const result = authManager.registerStudent(name, rollNo, branch, email, securityQuestion, securityAnswer, password);
            if (!result.success) {
    ...
    if (studentLoginForm) {
        studentLoginForm.addEventListener('submit', (e) => {
            ...
            const result = authManager.loginStudent(rollNo, password);
            if (!result.success) {
                showToast(result.message, 'error');
                return;
            }
...
function initForgotPassword() {
    ...
    const goToStep = (step) => {
        ...
        step3Form.style.display = step === 3 ? 'block' : 'none';
    ...
    if (step1Form) {
        step1Form.addEventListener('submit', (e) => {
            ...
            const res = authManager.getSecurityQuestion(roll);
            ...
            document.getElementById('forgotCandidateName').textContent = res.name;
            document.getElementById('forgotCandidateMeta').textContent = `${res.rollNo} • Department of ${res.branch}`;
            document.getElementById('forgotSecurityQuestionDisplay').textContent = res.securityQuestion;
    ...
    if (step2Form) {
        step2Form.addEventListener('submit', (e) => {
            ...
            const verifyRes = authManager.verifyRecoveryDetails(forgotPasswordState.rollNo, answer);
    ...
    if (step3Form) {
        step3Form.addEventListener('submit', (e) => {
            ...
            const resetRes = authManager.resetPassword(forgotPasswordState.rollNo, newPass);
            if (!resetRes.success) {
            ...
            document.getElementById('loginRoll').value = forgotPasswordState.rollNo;
...
function initAccountSettingsModal() {
    ...
    if (settingsBtn && settingsModal) {
        settingsBtn.addEventListener('click', () => {
            ...
            document.getElementById('settingsStudentMeta').textContent = `${student.rollNo} • Department of ${student.branch}`;
            document.getElementById('settingsRegisteredDate').textContent = `Member since: ${student.registeredAt || '2026-08-19'}`;
    ...
    if (changePassForm) {
        changePassForm.addEventListener('submit', (e) => {
            ...
            const res = authManager.changePassword(student.rollNo, currentPass, newPass);
            if (!res.success) {
                showToast(res.message, 'error');
                return;
            }
...
function renderStudentDashboard() {
    ...
    document.getElementById('dashStudentRoll').textContent = student.rollNo;
    document.getElementById('dashStudentBranch').textContent = student.branch;
    ...
    if (totalAttempts > 0) {
        ...
        document.getElementById('dashBestScore').textContent = `${bestScore} / 20`;
        document.getElementById('dashAvgScore').textContent = `${avgScore} / 20`;
        document.getElementById('dashLastAttemptDate').textContent = latestAttempt.date;
    } else {
        document.getElementById('dashBestScore').textContent = `N/A`;
        document.getElementById('dashAvgScore').textContent = `N/A`;
        document.getElementById('dashLastAttemptDate').textContent = `Never`;
    }

    // Render Previous Attempts History Table
...
function bindTestEventListeners() {
    ...
    if (backToDashboardFromResultBtn) backToDashboardFromResultBtn.addEventListener('click', () => {
        renderStudentDashboard();
        showScreen('dashboardView');
    });
    ...
    if (certBackBtn) {
        certBackBtn.addEventListener('click', () => {
            soundManager.playClick();
            // Return back to wherever the user came from (resultView or dashboardView)
...
function finishAndEvaluateTest() {
    if (AppState.timerInterval) clearInterval(AppState.timerInterval);
    ...
    const comparison = authManager.getScoreComparison(score, percentage);

    // Save full test snapshot (including questions & userAnswers)
    const savedAttempt = authManager.saveTestAttempt({
        score,
    ...
    document.getElementById('resScoreTotal').textContent = `/ ${totalQuestions}`;
    document.getElementById('resPercentageText').textContent = `${percentage}% Overall Score`;
    ...
    document.getElementById('statCorrectCount').textContent = correctCount;
    document.getElementById('statWrongCount').textContent = wrongCount;
    document.getElementById('statUnattemptedCount').textContent = unattemptedCount;
    document.getElementById('statAccuracyRate').textContent = `${accuracy}%`;
    document.getElementById('statTimeTaken').textContent = leaderboardManager.formatTime(timeSpentSeconds);
    document.getElementById('statLeaderboardRank').textContent = `#${userRank}`;
...
function renderCategoryBars(categoryStats) {
    ...
    document.getElementById('catQuantScore').textContent = `${quantStats.correct}/${quantStats.total} (${qPct}%)`;
    document.getElementById('catLogicScore').textContent = `${logicStats.correct}/${logicStats.total} (${lPct}%)`;
    document.getElementById('catVerbalScore').textContent = `${verbalStats.correct}/${verbalStats.total} (${vPct}%)`;

    setTimeout(() => {
        document.getElementById('catQuantFill').style.width = `${qPct}%`;
        document.getElementById('catLogicFill').style.width = `${lPct}%`;
        document.getElementById('catVerbalFill').style.width = `${vPct}%`;
    }, 200);
}
